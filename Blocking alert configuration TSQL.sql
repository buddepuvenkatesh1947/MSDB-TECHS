...
/****** Object:  StoredProcedure [dbo].[BlockingMonitor]    Script Date: 02-01-2022 02:26:03 PM ******/
SET ANSI_NULLS ON
GO
...
Where
Waits.wait_duration_ms / 1000 > 60  
and BlockedSess.login_name like '%testing01%' 
and Sess.login_name not like '%lenovo\sow_%'
Order By 
WaitInSeconds Desc;
/* Remove long SSRS long running report notifications */ 
Delete #Blocking Where BlockingSQL Like '%WriteLockSession%'
...
'<td> <font style="font-family:Verdana; font-size:12pt; width:auto; float:left; padding-left:5px; "> You will get an alert when blocking occurs by user "testing01" user only for more than 1 minute but does NOT start with lenovo\testing
If it is select command then can kill it.If it is other query then you have to drop an email
to application team apps@yourcompany.com.</font> </td>' +
'</tr>' +
'</table>' +
'<br>' +
